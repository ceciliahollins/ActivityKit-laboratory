...
struct MusicPlayerLiveActivity: Widget {
    ...
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: MusicPlayerAttributes.self) { context in
            ...
            VStack {
                createLockScreen(currSong: context.state.currSong)
            }
            ...
            DynamicIsland {
                ...
                DynamicIslandExpandedRegion(.leading) {
                    createExpandedLeading(currSong: context.state.currSong)
                }
                DynamicIslandExpandedRegion(.trailing) {
                    createExpandedTrailing()
                }
                DynamicIslandExpandedRegion(.bottom) {
                    createExpandedBottom(currSong: context.state.currSong)
                }
            } compactLeading: {
                createCompactLeading(currSong: context.state.currSong)
            } compactTrailing: {
                createCompactTrailing()
            } minimal: {
                createMinimal(currSong: context.state.currSong)
            }
